...
class WORLD(Tk):
    def __init__(self, queue):#漏参数queue
        Tk.__init__(self)
        self.queue =queue#漏 =queue
        self.is_game_over = False
        self.canvas = Canvas(self, width=500, height=300, bg='gray')
        self.canvas.pack()#漏掉这句
        #self, pack()
...
class Snake(threading.Thread):
    ...
    def check_game_over(self, snake_point):
            ...
            if not -5 < x < 505 or -5 < y < 315 or snake_point in self.snake_points:
